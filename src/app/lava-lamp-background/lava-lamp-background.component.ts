...
export class LavaLampBackgroundComponent implements AfterViewInit {
  @ViewChild('lavaCanvas', { static: false }) canvasRef!: ElementRef<HTMLCanvasElement>;

  ngAfterViewInit() {
    const canvas = this.canvasRef.nativeElement;

    // Paste the JavaScript code from the CodePen here,
    // replacing 'canvas' with this canvas element if needed
    this.initLavaLamp(canvas);
  }

  initLavaLamp(canvas: HTMLCanvasElement) {
    // Example from CodePen (simplified)
    ...
    // ...rest of the WebGL initialization and animation code...
    // Make sure to replace "document.querySelector('canvas')" with `canvas`
    // For example:
  ...
  void main(void) {
    ...
    function createShader(type: number, source: string): WebGLShader | null {
      ...
      if (!shader) return null; // safety check
